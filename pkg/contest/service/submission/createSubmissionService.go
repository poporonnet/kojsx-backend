...
func (s CreateSubmissionService) Handle(pID, cID id.SnowFlakeID, lang, code string) (*model.Submission, error) {
	newID := s.idGenerator.NewID(time.Now())

	d, err := model.NewSubmission(newID, pID, cID, lang, code, time.Now())
	if err != nil {
...
func (s CreateSubmissionService) CreateResult(submissionID id.SnowFlakeID, args []CreateResultArgs) (Data, error) {
	...
	results := make([]model.SubmissionResult, len(args))
	for i, v := range args {
		time.Sleep(2 * time.Millisecond)
		d := model.NewSubmissionResult(
