...
func (d Data) ToDomain() *model.Problem {
	dd := model.NewProblem(
		d.GetID(),
		d.GetContestID(),
	)
...
func DomainToData(in model.Problem) Data {
	return *NewData(
		in.GetProblemID(),
		in.GetContestID(),
		in.GetIndex(),
...
func (d CaseSetData) ToDomain() *model.Caseset {
	s := model.NewCaseset(d.id)
	_ = s.SetName(d.name)
...
func DomainToCaseSetData(in []model.Caseset) []CaseSetData {
...
func (d CaseData) ToDomain() *model.Case {
	c := model.NewCase(d.id, d.caseSetID)
	_ = c.SetIn(d.in)
	_ = c.SetOut(d.out)
	return c
}
